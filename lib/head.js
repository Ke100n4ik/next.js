...
function unique () {
  ...
  return (h) => {
    switch (h.type) {
      case 'title':
      case 'style':
        if (h.props.hasOwnProperty('dangerouslySetInnerHTML')) {
          const innerHTML = h.props.dangerouslySetInnerHTML;
          if (innerHTML.hasOwnProperty('__html') && innerHTML.__html) {
            const style = innerHTML.__html;
